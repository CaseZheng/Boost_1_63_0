...
int main (int argc, char **argv) {
    ...
    char *shortopts =
          ...
          "p:"  /* TCP port number to listen on 设置Memcached TCP监听的端口*/
          "s:"  /* unix socket path to listen on */
          "U:"  /* UDP port number to listen on */
          "m:"  /* max memory to use for items in megabytes 分配给Memcached使用的内存数量，单位是MB*/
          "M"   /* return error on memory exhausted */
          "c:"  /* max simultaneous connections 最大运行的并发连接数*/
          "k"   /* lock down all paged memory */
          "hiV" /* help, licence info, version */
          "r"   /* maximize core file limit */
          "v"   /* verbose */
          "d"   /* daemon mode 守护进程*/
          "l:"  /* interface to listen on 监听的服务器IP地址*/
          "u:"  /* user identity to run as 运行Memcached的用户*/
          "P:"  /* save PID in file 保存Memcached的pid文件*/
          "f:"  /* factor? */
